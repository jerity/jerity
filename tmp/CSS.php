...
class CSSCruncher {
	...
	public function readFiles() {
		...
		foreach($this->dirs as $name) {
			...
			if (beginsWith($name, $this->styles_folder)) {
              ...
              if ( endsWith(strtolower($file), $typeext . '.css')
                && !(beginsWith($entry, '!')) && !( ($this->type == 'styles')
                && (substr_count($entry, '.') > 1))) {
								$df[$entry] = $file;
							}
						}
					}
	...
	protected function importFile($css_key, $file_key, $file_path) {
		...
		if (!isset($this->css[$css_key])) $this->css[$css_key] = array();
		if (!isset($this->css[$css_key][$type])) $this->css[$css_key][$type] = '';
	...
	public function writeFiles() {
		...
		foreach($this->css as $dir_name => $types) {
			if ($dir_name == '_main') $dir_name = '';
			if (substr($dir_name, 0, 1) == '_') $dir_name = substr($dir_name, 1);
